...
| Date | July 13, 2026 |
| Deciders | @elainechan, Contributors (proposed; verify and accept the infrastructure decision); Tapestry Governance Committee (to ratify the governance-owned elements: membership admission and contribution weighting) |
...
The central node is a **governed coordination role realized as a memory-bound service, not a GPU training cluster, and not a control point.** Three governance properties bind the infrastructure and take precedence over any convenience below to make TAP-004's "governed role, not a power center" enforceable in the build:
...
- **2. No unilateral control.** 
    - a. Neither the node operator nor any single member decides who is admitted or how contributions are weighted. 
    - b. The infrastructure *enforces* admission and weighting decisions made by consortium governance; it does not *make* them ([TAP-002](adr-002-consortium-training.md) shared governance; [TAP-004](adr-004-training-loop.md) open question 2).
- **3. Auditable.** Every admission, aggregation, and weighting decision is signed and written to an append-only sink open to consortium oversight ([TAP-008](adr-008-data-sovereignty.md)).
...
**5. Secure integration and logging.** One narrow, mutually-authenticated ingress per member, with schema validation that rejects anything that is not a recognized signed checkpoint manifest. **Membership admission is a consortium-governance decision, not the operator's:** the network/IAM layer *enforces* an admission list that governance controls and must not let whoever holds the hosting account unilaterally add or exclude members. Building Flower's (NoOp-in-open-source) `FederationManager` is deferred for the two known M0 members, but the deferral is **conditional on admission control remaining with governance** rather than defaulting to the operator. Status and logs aggregate to the append-only oversight sink: each node ships round status; the coordinator logs each aggregation decision, the weighting applied, and eval outcomes — satisfying the issue's logging requirement and [TAP-008](adr-008-data-sovereignty.md)'s enforcement-audit obligation. *Reference (AWS):* PrivateLink/VPN + mTLS ingress; a governance-owned IAM/SCP admission boundary; cross-account S3 (Object Lock) + CloudWatch/CloudTrail for the audit sink.
...
1. Consortium governance (TAP-002; TAP-004 Phase 5, Decision 7) ratifies the governance-owned elements this ADR only *enforces*: that membership admission and contribution weighting are governance decisions, and that hosting the node confers no admission or weighting authority.
...
- **Build `FederationManager` now for membership.** Deferred: with two known members, a governance-owned admission list enforced at the network/IAM layer is sufficient and simpler. The deferral is conditional — it holds only while admission control stays with governance and does not concentrate in the operator — and is revisited as N grows.
...
- Mandate Flower. It recommends Flower's SuperLink as the substrate on the compatibility review, but the decision that binds is the coordinator's *responsibilities, sizing, and governance properties*; a conformant from-scratch coordinator is not precluded.
...
| :------- | :---------- | :-------------- |
| [TAP-002](adr-002-consortium-training.md) | Establishes "consortium with shared ownership and governance rights" and members' architectural voice | Record that membership admission and contribution weighting are governance-owned; the central node's infrastructure enforces those decisions and must not let its operator make them, so hosting the node confers no admission or weighting authority |
| [TAP-004](adr-004-training-loop.md) | States the coordinator is "a governed role, not a power center" (Consequences; Phase 5, Decision 7) and flags weighting as an open optimization-and-governance question (OQ2) | Record that this ADR treats the coordinator as a governed role — building weighting as a governance-configurable, conservatively-defaulted mechanism rather than resolving it — and names Phase 5, Decision 7 as the governance owner of admission and weighting |
